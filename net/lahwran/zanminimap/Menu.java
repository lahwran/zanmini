...
public class Menu {
    ...
    Menu(ZanMinimap minimap) {
        ...
        this.sMenu[0][3] = "- Press §B" + Keyboard.getKeyName(conf.zoomKey)
                + " §Fto zoom in/out, or §B" + Keyboard.getKeyName(conf.menuKey)
                + "§F for options.";
    ...
    void showMenu(int scWidth, int scHeight) {
        ...
        if (this.iMenu == 1) {
            ...
            botY = centerY + (height - 1) / 2.0D * 10.0D + border;
            this.drawBox(leftX, rightX, topY, botY);
            this.drawOptions(rightX - border,
                    topY + border,
                    MouseX,
                    MouseY,
                    leftclick,
                    rightclick,
                    middleclick);
            footer = this.drawFooter(centerX,
                    centerY,
                    height,
                    opt1,
                    opt2,
        ...
        if (this.iMenu == 1) {
            for (int n = 1; n < height; n++)
                obfhub.write(this.sMenu[iMenu - 1][n],
                        centerX - maxSize / 2,
                        ((centerY - (height - 1) * 10 / 2) + (n * 10)) - 9,
                        0xffffff);
            ...
            if (this.iMenu == 2) {
                for (int n = 1; n < height; n++) {
                    ...
                    obfhub.write(hide, (int) rightX - border - 15 - obfhub.calcStringLength(hide)
                            / 2, ((centerY - (height - 1) * 10 / 2) + (n * 10)) - 8, 0xffffff);
                }
            } else {
                int max = min + 9;
                ...
                for (int n = min; n < max; n++) {
                    ...
                    obfhub.write((n + 1) + ") " + conf.wayPoints.get(n).name,
                            leftTxt,
                            yTop - 9,
                            0xffffff);
                    ...
                    obfhub.write(hide, (int) rightX - border - 29 - obfhub.calcStringLength(hide)
                            / 2, yTop - 8, 0xffffff);

                    if (MouseX > leftTxt && MouseX < (rightX - border - 77) && MouseY > yTop - 10
                            && MouseY < yTop - 1) {
            ...
            if (this.iMenu == 2) {
                obfhub.write(opt1, centerX - 5 - border - footer - obfhub.calcStringLength(opt1)
                        / 2, footpos, 16777215);
                obfhub.write(opt2, centerX + border + 5 + footer - obfhub.calcStringLength(opt2)
                        / 2, footpos, 16777215);
            } else {
                if (this.iMenu != 4)
                    obfhub.write(opt1, centerX - 5 - border * 2 - footer * 2
                            - obfhub.calcStringLength(opt1) / 2, footpos, 16777215);

                obfhub.write(opt2, centerX - obfhub.calcStringLength(opt2) / 2, footpos, 16777215);

                if (this.iMenu != 4)
                    obfhub.write(opt3, centerX + 5 + border * 2 + footer * 2
                            - obfhub.calcStringLength(opt3) / 2, footpos, 16777215);
            }
        }
            ...
            if (Keyboard.getEventKeyState()) {
                do {
                    if (Keyboard.getEventKey() == Keyboard.KEY_RETURN
                            && this.lastKey != Keyboard.KEY_RETURN)
                        if (this.inStr.equals(""))
                            ...
                            if (!this.inStr.equals("")) {
                                ...
                                if (conf.wayPoints.size() > 9)
                                    min = conf.wayPoints.size() - 9;
                            }
                        }
                    else if (Keyboard.getEventKey() == Keyboard.KEY_BACK
                            && this.lastKey != Keyboard.KEY_BACK)
                        if (this.inStr.length() > 0)
                            this.inStr = this.inStr.substring(0, this.inStr.length() - 1);

                    if (verify.indexOf(Keyboard.getEventCharacter()) >= 0
                            && Keyboard.getEventKey() != this.lastKey)
    ...
    private void drawOptions(double rightX, double topY, int MouseX, int MouseY, boolean leftclick,
            boolean rightclick, boolean middleclick) {
        if (this.iMenu > 2) {
            if (min < 0)
                min = 0;
            ...
            if (MouseX > (rightX - 10) && MouseX < (rightX - 2) && MouseY > (topY + 1)
                    && MouseY < (topY + 10)) {
                if (leftbtndown) {
                    if (leftclick && min > 0)
                        min--;
            ...
            if (MouseX > rightX - 10 && MouseX < rightX - 2 && MouseY > topY + 12 + scrollMin
                    && MouseY < topY + 12 + scrollMin + scrollMax || scrollClick) {
                if (Mouse.isButtonDown(0) && !scrollClick) {
                    scrollClick = true;
                    scrollStart = MouseY;
            ...
            this.drawBox(rightX - 10, rightX - 2, topY + 12 + scrollMin, topY + 12 + scrollMin
                    + scrollMax);

            if (MouseX > rightX - 10 && MouseX < rightX - 2 && MouseY > topY + 81
                    && MouseY < topY + 90) {
                if (leftbtndown) {
                    if (leftclick && min < conf.wayPoints.size() - 9)
                        min++;
        ...
        for (int i = min; i < max; i++) {
            ...
            if (iMenu > 2 && !(iMenu == 4 && this.next == 3)) {
                if (MouseX > leftCl && MouseX < rightCl && MouseY > topY && MouseY < botY
                        && this.iMenu == 3)
                ...
                GL11.glColor3f(conf.wayPoints.get(i).red,
                        conf.wayPoints.get(i).green,
                        conf.wayPoints.get(i).blue);
    ...
    private int drawFooter(int centerX, int centerY, int m, String opt1, String opt2, String opt3,
            int border, int MouseX, int MouseY, boolean leftclick) {
        int footer = obfhub.calcStringLength(opt1);

        if (obfhub.calcStringLength(opt2) > footer)
            footer = obfhub.calcStringLength(opt2);
        ...
        if (this.iMenu > 2) {
            if (obfhub.calcStringLength(opt3) > footer)
                footer = obfhub.calcStringLength(opt3);
    ...
    private boolean chkOptions(int i) {
        if (i == 0)
            ...
            return conf.lightmap;
        else if (i == 3)
            return conf.heightmap;
        else if (i == 4)
            return conf.squaremap;
        else if (i == 5)
            return conf.welcome;
        else if (i == 6)
    ...
    void onRenderTick(int scWidth, int scHeight) {
        ...
        if (conf.enabled) {
            if (ztimer > 0)
                obfhub.write(error, 20, 20, 0xffffff);
